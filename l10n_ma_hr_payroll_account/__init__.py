...
def _set_accounts(cr, registry):
    return True
    #write the default debit account on salary rule having xml_id like 'l10n_ma_hr_payroll.1' up to 'l10n_ma_hr_payroll.1409'
    env = api.Environment(cr, SUPERUSER_ID, {})
    names = [str(x) for x in range(1,1410)]
    data = env['ir.model.data'].search([('model', '=', 'hr.salary.rule'), ('module', '=', 'l10n_ma_hr_payroll'), ('name', 'in', names)])
