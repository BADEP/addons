...
class CrmFacebookForm(models.Model):
    ...
    team_id = fields.Many2one('crm.case.section', domain=['|', ('use_leads', '=', True), ('use_opportunities', '=', True)], string="Sales Team")
    campaign_id = fields.Many2one('crm.tracking.campaign')
    source_id = fields.Many2one('crm.tracking.source')
    medium_id = fields.Many2one('crm.tracking.medium')
                ...
                for lead in r['data']:
                    if not self.search([('facebook_lead_id', '=', lead.get('id')), '|', ('active', '=', True), ('active', '=', False)]):
                        ...
                        vals.update({
                            ...
                            'description': "\n".join(notes),
                            'section_id': form.team_id and form.team_id.id,
